...
def reset_database():
	...
	doc = frappe.new_doc("Deletion Record")
	doc.posting_date = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
	doc.deleted_by = frappe.session.user
	doc.note = "User {0} Reset Database".format(doc.deleted_by)
	doc.docstatus = 1
	doc.insert()
...
def delete_transactions():
	...
	doc.posting_date = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
	doc.deleted_by = frappe.session.user
	doc.note = "User {0} Delete Transactions".format(doc.deleted_by)
	doc.docstatus = 1
	doc.insert()
